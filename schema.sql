...
ALTER TABLE `customers` 
    -- Đặt mặc định tier_id là 1
    MODIFY COLUMN `tier_id` int DEFAULT 1,
    -- Thêm cột username (duy nhất để đăng nhập)
    ADD COLUMN `username` varchar(50) UNIQUE AFTER `tier_id`,
    -- Đảm bảo cột password tồn tại và có độ dài phù hợp cho mã hóa (VD: Bcrypt)
    MODIFY COLUMN `password` varchar(255) NOT NULL;

-- Cập nhật lại các ràng buộc khóa ngoại nếu cần đảm bảo tính nhất quán
ALTER TABLE `customers`
    DROP FOREIGN KEY `customers_ibfk_1`;

ALTER TABLE `customers`
    ADD CONSTRAINT `customers_ibfk_1` FOREIGN KEY (`tier_id`) REFERENCES `customer_tiers` (`id`) ON DELETE SET DEFAULT;